...
const FIBER_METHODS: Record<string, HttpMethod> = {
  Get: 'GET', Post: 'POST', Put: 'PUT', Delete: 'DELETE',
  Patch: 'PATCH', Head: 'HEAD', Options: 'OPTIONS',
}

export class FiberExtractor extends BaseApiExtractor {
  readonly extractorId = 'go.fiber'
  ...
  async extract(): Promise<ApiExtractorResult> {
    ...
    for (const file of uniqueFiles) {
      try {
        ...
        const parsed = parseFiberFile(content, file)
...
function parseFiberFile(content: string, file: string): RawApiSurface | null {
  const routerVars = detectFiberVars(content)
  ...
  const methods = Object.keys(FIBER_METHODS).join('|')
  ...
  for (let i = 0; i < lines.length; i++) {
    ...
    const httpMethod = FIBER_METHODS[m[1]]
...
function detectFiberVars(content: string): Set<string> {
  const vars = new Set<string>()
  for (const m of content.matchAll(/(\w+)\s*:?=\s*fiber\.New\s*\(/g)) vars.add(m[1])
  // Groups: api := app.Group("/api")
  ...
  for (const m of content.matchAll(/\b(\w+)\.(Get|Post|Put|Delete|Patch|Head|Options)\s*\(\s*"/g)) {
    if (!['c', 'ctx', 'req', 'resp'].includes(m[1])) vars.add(m[1])
  }
  return vars
}
